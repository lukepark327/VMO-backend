...
function initHttpServer() {
    ...
    app.post("/mineBlock", function (req, res) {
        /**
         * "general" is default category.
         * The first element of data array must be a category label.
         */
        const data = req.body.data || ["general"];
    ...
    app.get('/category/:filter', function (req, res) {
        try {
            const targetBlock = getBlockchain().blocks.filter(function (block) {
                return block.data[0] === req.params.filter;
            });
            res.send(targetBlock);
        }
        catch (err) {
